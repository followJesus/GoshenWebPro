...
/*模拟下拉菜单*/
jQuery.gaoshenselect = function(divselectid,inputselectid) {
...
/*折叠*/
jQuery.gaoshenfold = function(obj,obj_c,speed,obj_type,Event){
	if(obj_type == 2){
...
/*返回顶部*/
var $backToTopEle=$('<a href="javascript:void(0)" class="goshen-iconfont toTop" title="返回顶部" alt="返回顶部" style="display:none">&#xe684;</a>').appendTo($("body")).click(function(){
...
/*gaoshenmodalalert*/
function gaoshenmodal_alert(info,speed){
	if(speed==0||typeof(speed) == "undefined"){
		...
		$("#modal-alert").fadeIn();
		setTimeout("gaoshenmodal_alert_hide()",speed);
	}	
}
function gaoshenmodal_alert_hide() {
...
$(function(){
	/*****表单*****/
    $.gaoshenfocusblur(".input-text,.textarea");
	/*按钮loading*/
	...
	/**/
	$.gaoshenselect("#divselect","#inputselect");
	...
	/*gaoshenalert*/
	$.gaoshenhover('.gaoshenalert i');
	$(".gaoshenalert i").on("click",function(){
		var gaoshenalert = $(this).parents(".gaoshenalert");
		gaoshenalert.fadeOut("normal",function(){
		  gaoshenalert.remove();
		});
	});
	...
	/*tag标签*/
	var time1;
	$(".gaoshen-tags-lable").show();
	$(".gaoshen-tags-input").val("");
	$(document).on("blur",".gaoshen-tags-input",function(){
		time1 = setTimeout(function(){
			$(this).parents(".gaoshen-tags").find(".gaoshen-tags-list").slideUp();
		}, 400);
	});
	$(document).on("focus",".gaoshen-tags-input",function(){
		clearTimeout(time1);
	});
	$(document).on("click",".gaoshen-tags-input",function(){
		$(this).find(".gaoshen-tags-input").focus();
		$(this).find(".gaoshen-tags-list").slideDown();
	});
	function gettagval(obj){
		var str ="";
		var token =$(obj).parents(".gaoshen-tags").find(".gaoshen-tags-token");
		//alert(token.length)
		if(token.length<1){
			$(obj).parents(".gaoshen-tags").find(".gaoshen-tags-val").val("");
			return false;
		}
		for(var i = 0;i< token.length;i++){
			str += token.eq(i).text() + ",";
			$(obj).parents(".gaoshen-tags").find(".gaoshen-tags-val").val(str);
		}
	}
	$(document).on("keydown",".gaoshen-tags-input",function(event){
		$(this).next().hide();
		var v = $(this).val().replace(/\s+/g, "");
		var reg=/^,|,$/gi;
		v=v.replace(reg,"");
		v=$.trim(v);
		var token =$(this).parents(".gaoshen-tags").find(".gaoshen-tags-token");
		if(v!=''){
			if(event.keyCode==13||event.keyCode==108||event.keyCode==32){
				$('<span class="gaoshen-tags-token">'+v+'</span>').insertBefore($(this).parents(".gaoshen-tags").find(".gaoshen-tags-iptwrap"));
				$(this).val("");
				gettagval(this);
			}
		}else{
			if(event.keyCode==8){
				if(token.length>=1){
					$(this).parents(".gaoshen-tags").find(".gaoshen-tags-token:last").remove();
					gettagval(this);
				}
				else{
					$(this).parents(".gaoshen-tags").find(".gaoshen-tags-lable").show();
					return false;
				}
	...
	$(document).on("click",".gaoshen-tags-has span",function(){
		var taghasV = $(this).text();
		taghasV=taghasV.replace(/(^\s*)|(\s*$)/g,"");
		$('<span class="gaoshen-tags-token">'+taghasV+'</span>').insertBefore($(this).parents(".gaoshen-tags").find(".gaoshen-tags-iptwrap"));
		gettagval(this);
		$(this).parents(".gaoshen-tags").find(".gaoshen-tags-input").focus();
	});
	$(document).on("click",".gaoshen-tags-token",function(){
		var token =$(this).parents(".gaoshen-tags").find(".gaoshen-tags-token");
		var it = $(this).parents(".gaoshen-tags");
		$(this).remove();
		switch(token.length){
			case 1 : it.find(".gaoshen-tags-lable").show();
			break;
		}
		var str ="";
		var token =it.find(".gaoshen-tags-token");
		//alert(token.length)
		if(token.length<1){
			it.find(".gaoshen-tags-val").val("");
			return false;
		}
		for(var i = 0;i< token.length;i++){
			str += token.eq(i).text() + ",";
			it.find(".gaoshen-tags-val").val(str);
		}		
	});
});
